...
public class selenium extends testaction {
	...
	public result validate() {
		...
		if (actname.equals("browser.back") ||
				actname.equals("checkfileexists")||
		...
		else if (actname.equals("checkfileexists")||actname.equals("openbrowserproxy")) {
			return validate_has_5_input();
		}
		return new result(status_run.STOP, error_code.NOT_SUPPORT, "Action '" + get_name() + "' is not supported by Selenium runtime3");
	}
	...
	public result execute(int timeout) {
		...
		try {
			...
			else if (actname.equals("checkfileexists"))
				run_res = execute_checkFileExistAfterDownload();
			else
				run_res = new result(status_run.STOP, error_code.NOT_SUPPORT, "Action '" + get_name() + "' is not supported by Selenium runtime2");
